...
# Program Metrics 
We compiled metadata from each of the four programs and facilitated a discussion on metric comparability. We built a controlled vocabularly using previously published literature and expert opinion.    
...
```
# ScienceBase 
We use ScienceBase to document the metadata about these datasets. To run the following code you need to have a ScienceBase account. Once we get the ScienceBase documentation approved by the USGS the documentation will be accessable via APIs without credentials.  
...
NRSA_data <- download_EPA_NRSA(SBUserName,SBPassword, CRS)
...
Using the controlled vocabulary and the data exchange standards documented on GitHub here:https://github.com/rascully/Integrating-Stream-Monitoring-Data-From-Multiple-Programs. 

Run the following code to integrate the reformated data sets into an integrated data sets. 
...
pal <- colorFactor(c("green","red","blue", "orange"), domain=c("NRSA", "AREMP", "AIM", "PIBO"))

m <- data$SamplingFeature%>% 
      leaflet() %>%
      addTiles() %>%
      addCircles(lng=~verbatimLongitude, lat= ~verbatimLatitude, color=~pal(Program), 
                 popup= ~paste0("<b>",Program, "</b>", 
                                "<HR>", "<b>", "SamplingFeatureID ", "</b>",  SamplingFeatureID, "</HR>",
                                        "<br>")) %>%
                                addLegend("topleft", pal=pal, values= ~Program, opacity =1)

m

countEvents <- data$Action %>%
  group_by(Program)%>%
  count("Program") 

countLocations<- data$SamplingFeature %>%
 group_by(Program)%>%
 count("Program") 
...
The data tables are save to ScienceBase and in the data folder of this repository title [Integrated Data Set.xlsx](https://github.com/rascully/Integrating-Stream-Monitoring-Data-From-Multiple-Programs/blob/master/Data/Integrated%20Data%20Set.xlsx)
